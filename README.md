...
- models folder: contain `train_classifier.py` that use information from the database and create classifier for disaster messages, while saving best model to a `*.joblib` file
...
- LICENSE file
- README.md file
- requirement.txt file: list of programs to be pip installed in order to run the app
- Disaster_response_app folder: this folder contain all the files needed to create the app online (Heroku server). It is important to note that while main structure of repository supports app on the local host, the layout for hosting the app on a server require additional files and changed to `*.py` file. In case interested in necessary layout for online hosting: for more info see end of the README.md file
...
- Create a virtual environment where `requirements.txt` is located: 
```
conda update python
python3 -m venv name_of_your_choosing
source name_of_your_choosing/bin/activate
...
IMPORTANT: In addition of creating Procfile, \_\_init\_\_ file, and modifying several other file including run.py, disasterresponse.py, within `wrangling_scripts/wrangle_data.py` following class is added:
...
model = CustomUnpickler(open('files/best_model_serial_wtokenizer.pkl', 'rb')).load()
 ```   
 Without this class model built using train_classifer_pkl.py even though successfully generate a working model (with tokenizer) in local host, it can not be deployed. The reason is due to pickle function hierarchy and the fact that creating pkl file within train_classifer_pkl.py happened in the \_\_main\_\_ function. When model deployed, the structure of the model pipeline is understood but not custom functions e.g.(tokenize) within them. In the app where the pkl file is loaded again, the app has no knowledge of \_\_main\_\_, and this type of error is reported `Can't get attribute 'tokenize' on <module '__main__'` By using a Custompickler function before loading the app, the class overwrite intrinsic pickle `find_class` function and provide it a direct address to tokenize function (within essential.py). So simply importing the tokenize on top of the wrangle_data.py was not enough. Need to also add that for consistency and due to sklearn warning only pickle dump and load are used (instead of joblib). Also see https://stackoverflow.com/questions/27732354/unable-to-load-files-using-pickle-and-multiple-modules/27733727#27733727
